...
`/generate-sync-pr` 로 직접 호출하거나 "sync PR 만들어줘" 류 요청 시 발동되는 스킬입니다. 특정 release 버전을 다른 브랜치(`master` / 다음 release / `feature/*/root`)로 동기화하는 **Sync PR** 을 만듭니다.
...
> **Draft → Ready 플로우**: PR을 먼저 `--draft` 로 생성합니다(리뷰어 자동 알림 방지). 생성 직후 `AskUserQuestion` 으로 "지금 review 를 open(Ready for review) 할까요?" 를 물어, open 선택 시 `gh pr ready <PR번호>` 로 Draft 를 해제합니다. Draft 유지 선택 시 PR 링크만 안내하고 종료합니다.
...
- 타겟은 인자 우선, 누락 시 `AskUserQuestion` 으로 (master / 다음 release / feature root) 선택.
...
| 단계 | 내용 |
|---|---|
| 0 | 도구 사전 준비 — `AskUserQuestion`·GitHub MCP 스키마 로드 (`ToolSearch`) |
...
| 5 | push |
| 6 | 제목 생성 (본문 비움) |
| 7 | **사용자 확인(`AskUserQuestion`) 후** `gh pr create --draft` |
